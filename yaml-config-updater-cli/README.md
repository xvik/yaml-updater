...
The first two are [attached to release](https://github.com/xvik/yaml-updater/releases/tag/1.4.3).
...
Jar could be downloaded from [github release](https://github.com/xvik/yaml-updater/releases/tag/1.4.3)
or from [maven central](https://repo1.maven.org/maven2/ru/vyarus/yaml-config-updater-cli/1.4.3/) (classifier `all`)
...
```groovy
implementation 'ru.vyarus:yaml-config-updater-cli:1.4.3'
```
...
Native binaries are attached to [github release](https://github.com/xvik/yaml-updater/releases/tag/1.4.3).
...
```
docker run -it --rm ghcr.io/xvik/yaml-updater:1.4.3 --version
```
...
```
docker run -it --rm -v $(pwd):/tmp/data ghcr.io/xvik/yaml-updater:1.4.3 /tmp/data/config.yml /tmp/data/update.yml
```
...
```
docker run -it --rm -v "C:/data":/tmp/data ghcr.io/xvik/yaml-updater:1.4.3 /tmp/data/config.yml /tmp/data/update.yml
```
